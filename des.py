...
def string_bit_to_bit_array(text):
    array = list()
    for char in text:
        binval = int(char)
        array.append(binval)
...
class des():
    def __init__(self,password=None,text=None):
    ...
    def generatekeys(self):
        self.keys = []
        key = string_bit_to_bit_array(self.password)
        ...
        for i in range(1):
            ...
            print('key')
            print(self.permut(tmp, CP_2)) #key

    def process(self):
        self.generatekeys()
        text_blocks = self.text
        result = list()
    
        block = string_bit_to_bit_array(text_blocks)
        ...
        print('L1||R1')
        print(self.permut(d+g, PI_1)) #encrypt result
